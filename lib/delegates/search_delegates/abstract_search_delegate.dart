...
/// This is an adaptation of the default [SearchDelegate] to allow for more customization.
...
/// [PIPSearchDelegate.close] call. If the search page is closed with the system
/// back button, it returns null.
///
/// A given [PIPSearchDelegate] can only be associated with one active [showSearch]
/// call. Call [PIPSearchDelegate.close] before re-using the same delegate instance
...
/// The animation provided by [PIPSearchDelegate.transitionAnimation] can be used
...
/// See also:
///
///  * [PIPSearchDelegate] to define the content of the search page.
Future<T?> showPIPCustomSearch<T>({
  required BuildContext context,
  required PIPSearchDelegate<T> delegate,
  String? query = '',
...
/// query text field can be customized via [PIPSearchDelegate.buildLeading]
/// and [PIPSearchDelegate.buildActions]. Additionally, a widget can be placed
/// across the bottom of the [AppBar] via [PIPSearchDelegate.buildBottom].
///
/// The body below the [AppBar] can either show suggested queries (returned by
/// [PIPSearchDelegate.buildSuggestions]) or - once the user submits a search  - the
/// results of the search as returned by [PIPSearchDelegate.buildResults].
///
/// [PIPSearchDelegate.query] always contains the current query entered by the user
/// and should be used to build the suggestions and results.
///
/// The results can be brought on screen by calling [PIPSearchDelegate.showResults]
/// and you can go back to showing the suggestions by calling
/// [PIPSearchDelegate.showSuggestions].
///
/// Once the user has selected a search result, [PIPSearchDelegate.close] should be
...
/// ## Handling emojis and other complex characters
/// {@macro flutter.widgets.EditableText.onChanged}
abstract class PIPSearchDelegate<T> {
  ...
  /// {@tool snippet}
  /// ```dart
  /// class CustomSearchHintDelegate extends PIPSearchDelegate<String> {
...
enum _SearchBody {
  ...
  ///
  /// The suggested queries are generated by [PIPSearchDelegate.buildSuggestions].
  suggestions,

  /// Search results are currently shown in the body.
  ///
  /// The search results are generated by [PIPSearchDelegate.buildResults].
  results,
}
...
class _SearchPageRoute<T> extends PageRoute<T> {
  _SearchPageRoute({
    ...
    assert(
      ...
      'search. Please close that search by calling close() on the PIPSearchDelegate '
      'before opening another search with the same delegate instance.',
    );
    delegate._route = this;
  }

  final PIPSearchDelegate<T> delegate;
              ...
              Container(
                decoration: BoxDecoration(
                  gradient: LinearGradient(
                    ...
                    colors: [
                      Color(int.parse(Provider.of<ColorANDLogoProvider>(context)
                          .primaryColor)),
                      Color(int.parse(Provider.of<ColorANDLogoProvider>(context)
                          .secondaryColor))
                    ],
                  ...
                  AppBar(
                    ...
                    title: TextField(
                      ...
                      decoration: InputDecoration(
                          // prefixIcon: widget.delegate.buildLeading(context),
